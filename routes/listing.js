...
router.route("/")
    ...
    .post(isLoggedIn,upload.single('image'),validateListing, wrapAsync(listtingController.createListing));
    //new route
router.get("/new", isLoggedIn, wrapAsync(listtingController.rendernewForm));

router.route("/:id")
    .get(wrapAsync(listtingController.showsallListings))
    .put(isLoggedIn, isOwner,upload.single('image'), validateListing, wrapAsync(listtingController.updateListing))
